...
def LBL(OPEN, CLOSE, HIGH, LOW, N=3):               #LBL 轨道线（文化财经公式）
...
def SAR(HIGH, LOW, N=4, STEP=2, MVALUE=20):         #抛物转向（文化财经参数风格）
    ...
    n = max(int(N), 2)
    step = STEP / 100.0 if STEP > 1 else float(STEP)
    mvalue = MVALUE / 100.0 if MVALUE > 1 else float(MVALUE)
    step = max(step, 0.0001)
    mvalue = max(mvalue, step)

    sar = np.full(length, np.nan, dtype=float)
    init_idx = min(n - 1, length - 1)

    mid = (high + low) / 2.0
    trend_up = True if init_idx < 1 else np.nanmean(np.diff(mid[: init_idx + 1])) >= 0

    if trend_up:
        ep = np.nanmax(high[: init_idx + 1])
        sar[init_idx] = np.nanmin(low[: init_idx + 1])
    else:
        ep = np.nanmin(low[: init_idx + 1])
        sar[init_idx] = np.nanmax(high[: init_idx + 1])
    ...
    for i in range(init_idx + 1, length):
        ...
        if trend_up:
            ...
            if low[i] < val:
                ...
                if high[i] > ep:
                    ep = high[i]
                    af = min(af + step, mvalue)
        else:
            val = prev + af * (ep - prev)
            if i >= 2:
                val = max(val, high[i - 1], high[i - 2])
            else:
