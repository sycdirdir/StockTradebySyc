...
## 前端可视化界面 (quant-trading-view)

本项目包含一个独立的 Vue3 前端，用于可视化 K 线、买卖信号和技术指标。

### 技术栈

- **前端**: Vue 3 (Composition API) + Vite 5 + ECharts 5 + Axios
- **后端**: Flask (Python) + PostgreSQL
...
│   ├── composables/useData.js  # K线数据 hooks
│   ├── components/
│   │   ├── KlineChart.vue   # ECharts K 线图
│   │   ├── TopBar.vue       # 顶部品牌栏
│   │   ├── StockInfoBar.vue # 实时行情信息
│   │   ├── ChartToolbar.vue # 周期/策略/指标工具栏
│   │   ├── LegendBar.vue    # 图例
│   │   ├── SidePanel.vue    # 策略统计+持仓+交易记录
│   │   └── BottomBar.vue    # 状态栏
│   └── styles/main.css     # CSS 设计令牌
├── package.json
└── vite.config.js          # API 代理配置

backend/                     # Flask 后端
├── app.py                  # RESTful API 主文件
...
|------|------|------|
| GET | /api/health | 健康检查 |
| GET | /api/stocks | 股票列表（支持关键词搜索） |
| GET | /api/stocks/:ts_code | 股票详情 |
| GET | /api/kline/:ts_code | K线 + 技术指标 + 买卖信号 |
...
- `stock_basic` — 股票基础信息
- `daily` — 日线数据（含 4Line 指标）
- `stock_weekly` — 周线数据
- `stock_monthly` — 月线数据
- `stock_1min/5min/30min/60min` — 分钟线数据
- `index_basic` — 指数基础信息
- `TargetList` — 选股目标表
