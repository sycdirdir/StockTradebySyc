...
VALID_FREQS = {"D", "W", "M", "1MIN", "5MIN", "15MIN", "30MIN", "60MIN"}


def _normalize_freq(freq: str) -> str:
    freq_raw = str(freq or "D").strip().upper()
    if freq_raw not in VALID_FREQS:
        raise ValueError(
            "不支持的 freq，允许值：D/W/M/1MIN/5MIN/15MIN/30MIN/60MIN"
        )
    return freq_raw


def _freq_folder_name(freq: str) -> str:
...
def _get_kline_tushare(code: str, start: str, end: str, freq: str) -> pd.DataFrame:
...
def fetch_one(
    ...
    for attempt in range(1, 4):
        try:
            new_df = _get_kline_tushare(code, start, end, freq)
            if new_df.empty:
...
# --------------------------- 主入口 --------------------------- #
def main():
    parser = argparse.ArgumentParser(description="从 stocklist.csv 读取股票池并用 Tushare 抓取K线（固定qfq，全量覆盖）")
    # 抓取范围
    ...
    parser.add_argument(
        "--freq",
        default="D",
        help="周期：D/W/M/1min/5min/15min/30min/60min（默认D）",
    )
    # 股票清单与板块过滤
    ...
    freq_folder = _freq_folder_name(freq)

    out_dir = Path(args.out) / freq_folder
    ...
    logger.info(
        "开始抓取 %d 支股票 | 数据源:Tushare(%s,qfq) | 日期:%s → %s | 排除:%s | 输出:%s",
        len(codes), freq, start, end, ",".join(sorted(exclude_boards)) or "无", out_dir,
    )
